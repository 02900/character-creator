        ...
        <div className="mt-6 w-full space-y-2">
          <div className="rounded-md bg-muted p-3">
            ...
            <div className="mt-2 text-xs space-y-1">
              <p><strong>Page Size:</strong> {config.pageSize}</p>
              <p><strong>Line Weight:</strong> {config.lineArt.lineWeight}</p>
              <p><strong>Character Position:</strong> {config.composition.characterPosition}</p>
              {config.effects?.magic && (
                <p><strong>Magic:</strong> {config.effects.magic.type} ({config.effects.magic.intensity})</p>
              )}
              {config.background?.sky && (
                <p><strong>Sky:</strong> {config.background.sky}</p>
              )}
            </div>
          </div>
